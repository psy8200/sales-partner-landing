...
async function restoreItemSettings() {
  try {
    console.log('🔍 아이템 설정 데이터 복구 시작...');
    ...
    console.log(`📊 복구할 아이템 설정 데이터: ${itemsData.length}건`);
    ...
    for (const itemData of itemsData) {
      try {
        ...
        await prisma.itemSetting.upsert({
          where: { id: processedItemData.id },
          update: processedItemData,
          create: processedItemData,
        });
        console.log(`✅ 아이템 설정 복구: ${itemData.productName} (${itemData.provider})`);
      } catch (itemError) {
        console.error(`❌ 아이템 설정 복구 실패 (${itemData.productName}):`, itemError.message);
      }
    }

    console.log(`🎉 아이템 설정 ${itemsData.length}건 복구 완료!`);
    
  } catch (error) {
    console.error('❌ 아이템 설정 데이터 복구 중 오류 발생:', error);
  } finally {
