...
export async function POST(req: Request) {
  try {
    ...
    console.log('🌐 [웹 로그인] 입력된 전화번호:', phone);
    console.log('🌐 [웹 로그인] 추출된 8자리:', phoneSuffix);
    ...
    console.log('🌐 [웹 로그인] 데이터베이스에서 사용자 조회 중...');
    ...
    console.log('🌐 [웹 로그인] 사용자 조회 결과:', user ? '사용자 발견' : '사용자 없음');

    if (!user) {
      console.log('🌐 [웹 로그인] 사용자를 찾을 수 없음:', { phone, phoneSuffix });
    ...
    // 로그인 성공 - 웹 전용 세션 토큰 생성
    const sessionToken = Buffer.from(JSON.stringify({
      userId: user.id,
      role: user.role,
      phone: user.phone,
      platform: 'web', // 웹 플랫폼 식별
      iat: Date.now()
    })).toString('base64url');
    ...
    console.log('🌐 [웹 로그인] 로그인 성공:', { userId: user.id, platform: 'web' });
