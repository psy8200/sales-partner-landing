...
/**
 * 선물 지급 처리 API
 * 승급회원의 선물을 지급 완료 처리합니다.
 */
export async function POST(request: NextRequest) {
  try {
    console.log('🔍 선물 지급 처리 API 호출 시작');
    
    const body = await request.json();
    const { promotionMemberId, processedBy, memo } = body;

    console.log('📋 요청 데이터:', { promotionMemberId, processedBy, memo });
    ...
    // 이미 지급 완료된 경우
    if (promotionMember.paymentStatus === 'PAID') {
      return NextResponse.json(
        { success: false, error: '이미 지급 완료된 선물입니다.' },
        { status: 409 }
      );
    }
    ...
    if (promotionMember.paymentStatus === 'CANCELLED') {
      return NextResponse.json(
        { success: false, error: '취소된 선물은 지급할 수 없습니다.' },
        { status: 409 }
      );
    }
    ...
    const updatedMember = await prisma.promotionMember.update({
      ...
      data: {
        ...
        memo: memo || promotionMember.memo
      }
    });

    console.log('✅ 선물 지급 완료:', {
    ...
    return NextResponse.json({
      success: true,
      data: updatedMember,
      message: '선물 지급이 완료되었습니다.'
    });

  } catch (error) {
    console.error('❌ 선물 지급 처리 오류:', error);
    return NextResponse.json(
      { success: false, error: '선물 지급 처리 중 오류가 발생했습니다.' },
      { status: 500 }
    );
  }
}
...
/**
 * 일괄 선물 지급 처리 API
 * 여러 승급회원의 선물을 일괄 지급 완료 처리합니다.
 */
export async function PUT(request: NextRequest) {
  try {
    console.log('🔍 일괄 선물 지급 처리 API 호출 시작');
    
    const body = await request.json();
    const { promotionMemberIds, processedBy, memo } = body;
    ...
    const promotionMembers = await prisma.promotionMember.findMany({
      where: {
        ...
        paymentStatus: 'PENDING' // 대기중인 것만 처리
      }
    });

    if (promotionMembers.length === 0) {
      return NextResponse.json(
        { success: false, error: '지급 가능한 승급회원이 없습니다.' },
        { status: 404 }
      );
    }
    ...
    return NextResponse.json({
      ...
      message: `${updateResult.count}명의 선물 지급이 완료되었습니다.`
    });

  } catch (error) {
    console.error('❌ 일괄 선물 지급 처리 오류:', error);
    return NextResponse.json(
      { success: false, error: '일괄 선물 지급 처리 중 오류가 발생했습니다.' },
      { status: 500 }
    );
  }
}
