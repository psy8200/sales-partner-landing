...
export async function POST(request: NextRequest) {
  try {
    ...
    // 수금관리계약에서 all-contracts로 되돌리기 (status를 'CONFIRMED'로 변경)
    const updatedContracts = await prisma.contract.updateMany({
      where: {
        id: {
          in: contractIds
        },
        status: 'COLLECTION' // 수금관리계약 상태에서만 되돌리기 가능
      },
      data: {
        status: 'CONFIRMED', // all-contracts 상태로 변경
        notes: '수금관리계약에서 all-contracts로 되돌려짐'
      }
    });

    if (updatedContracts.count === 0) {
      return NextResponse.json(
        { error: '되돌릴 수 있는 계약이 없습니다. (수금관리계약 상태의 계약만 되돌릴 수 있습니다)' },
        { status: 404 }
      );
    }
    ...
    console.error('all-contracts 되돌리기 오류:', error);
    return NextResponse.json(
