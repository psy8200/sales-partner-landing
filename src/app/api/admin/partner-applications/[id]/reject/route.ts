...
const rejectSchema = z.object({
  adminMemo: z.string().min(1, "반려 사유를 입력해주세요"),
});
...
export async function POST(
  ...
  try {
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      // 사용자 정보 업데이트 (partnerStatus를 NOT_APPLIED로 되돌림)
    ...
    console.error('파트너신청 반려 오류:', error);
    ...
    return NextResponse.json(
      { success: false, error: "파트너신청 반려에 실패했습니다." },
      { status: 500 }
    );
  }
}
