...
const AdminLayout = ({ children }: { children: React.ReactNode }) => {
  ...
  const allMenuItems = [
    ...
    { name: '상담관리', href: '/admin/consultation/requests', icon: '📋', requiredRole: null },
    { name: '계약관리', href: '/admin/contracts/entries', icon: '📝', requiredRole: null },
  ...
  const sideMenus: Record<string, { name: string; href: string; icon?: string }[]> = {
    ...
    '상담관리': [
      { name: '상담신청관리', href: '/admin/consultation/requests', icon: '📥' },
      { name: '상담이력관리', href: '/admin/consultation/consultations', icon: '📋' },
      { name: '상담통계', href: '/admin/consultation/consultations/stats', icon: '📊' },
    ],
    '계약관리': [
    ...
    '아이템관리': [
      ...
      { name: '즉시가입신청설정', href: '/admin/items/immediate-join', icon: '📝' },
      { name: '쇼핑몰구매설정', href: '/admin/items/shopping-mall', icon: '🛒' },
      { name: '상조결합상품설정', href: '/admin/items/funeral', icon: '⚰️' },
      { name: '보험상담설정', href: '/admin/items/insurance', icon: '🛡️' },
      { name: '인터넷+TV설정', href: '/admin/items/internet-tv', icon: '📺' },
      { name: '렌탈제품구매설정', href: '/admin/items/rental', icon: '📦' },
      { name: '렌탈몰분양가입설정', href: '/admin/items/rental-mall', icon: '🛒' },
    ],
    '백업관리': [
  ...
  function getSidebarItems() {
    // 상담관리 관련 경로인지 확인
    if (pathname?.startsWith('/admin/consultation')) {
      return sideMenus['상담관리'] || [];
    }
    ...
    // 아이템관리는 하드코딩된 메뉴만 사용 (동적 로딩 비활성화)
    if (key === '아이템관리') {
      return base;
    }
