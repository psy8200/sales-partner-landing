...
export default function CompletedContractsPage() {
  ...
  const fetchContracts = useCallback(async () => {
    try {
      ...
      const response = await fetch(`/api/admin/collections/completed-contracts?${params}`);
      if (response.ok) {
      ...
      console.error('수금완료계약 목록 조회 오류:', error);
    } finally {
      setLoading(false);
    }
  ...
  // 선택된 계약들을 일괄로 수금관리로 되돌리기 함수
  const handleBulkRevertToCollection = async () => {
    if (selectedContracts.size === 0) {
      alert('되돌릴 계약을 선택해주세요.');
      return;
    }
    ...
    try {
      ...
      const promises = selectedContractIds.map(contractId =>
        fetch(`/api/admin/contracts/${contractId}/revert-to-collection`, {
          method: 'POST',
          headers: {
      ...
      if (successCount > 0) {
        alert(`${successCount}개 계약이 수금관리계약으로 되돌려졌습니다.${failCount > 0 ? ` (${failCount}개 실패)` : ''}`);
        ...
        alert('계약 되돌리기에 실패했습니다.');
      }
    } catch (error) {
      console.error('일괄 되돌리기 오류:', error);
  ...
  const handleRevertToCollection = async (contractId: string) => {
    ...
    try {
      ...
      if (response.ok) {
        alert('계약이 수금관리계약으로 되돌려졌습니다.');
        
        // 되돌린 계약을 목록에서 제거 (즉시 UI 업데이트)
        setContracts(prevContracts => 
      ...
      <div className="w-full px-4 sm:px-6 lg:px-8 py-6">
        <div className="mb-6">
          <div className="flex items-center gap-3 mb-2">
            ...
            <h1 className="text-3xl font-bold text-gray-900">수금완료된계약</h1>
          </div>
          <p className="text-gray-600">수금이 완료된 계약 목록</p>
        </div>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 mb-6">
          <div className="flex flex-col sm:flex-row gap-4">
            ...
            <button
              onClick={handleBulkRevertToCollection}
              ...
              title="선택된 계약들을 수금관리계약으로 되돌리기"
            >
              <span>선택되돌리기</span>
              {selectedContracts.size > 0 && (
                <span className="bg-white text-blue-600 rounded-full px-2 py-1 text-xs font-semibold">
                ...
                {filteredContracts.length === 0 ? (
                  <tr>
                    <td colSpan={12} className="px-6 py-12 text-center text-gray-500">
                      ...
                      <p className="text-lg font-medium">수금완료된 계약이 없습니다</p>
                      <p className="text-sm">수금확정된 계약이 여기에 표시됩니다.</p>
                    </td>
                  </tr>
                ) : (
                    ...
                    return (
                      <tr key={contract.id} className="hover:bg-gray-50">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 w-20">
                          <button
                            onClick={() => handleRevertToCollection(contract.id)}
