...
const ImmediateJoinItemsPage = () => {
  ...
  const [pageTitle, setPageTitle] = useState('즉시가입설정'); // 기본값
  ...
  const [rows, setRows] = useState<ImmediateJoinRow[]>([]);

  // 페이지 제목을 서버에서 가져오기 (하드코딩된 제목)
  React.useEffect(() => {
    fetch('/api/admin/sidebar-items/%2Fadmin%2Fitems%2Fimmediate-join')
      .then(r => r.json())
      .then(d => {
  ...
  React.useEffect(() => {
    fetch('/api/admin/items/settings?category=INSTANT_PARTNER')
      .then(r=>r.json())
      .then(d=>{
        if (d?.items) {
  ...
  const handleAdd = () => {
    if (!rentalCompany.trim() || !productName.trim() || !paymentTerm.trim()) {
      alert('가입처, 상품명, 납입기간을 입력해주세요.');
      return;
    }
    ...
    const payload = {
      category: 'INSTANT_PARTNER',
      provider: rentalCompany.trim(),
    ...
    .then(r => r.json())
    .then(d => {
      console.log('서버 응답:', d); // 디버깅용
      if (d?.item) {
        setRows(prev=>[{
          id: d.item.id,
          rentalCompany: d.item.provider,
        ...
        alert('상품 추가에 실패했습니다: ' + (d?.error || '알 수 없는 오류'));
      }
    })
    .catch(error => {
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white shadow rounded-lg">
        <div className="p-4 space-y-4">
          {/* 1줄: 가입처 / 상품명 / 납입기간 / 기준금액 / 지급율 */}
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3">
            <div>
              <label className="block text-xs font-medium text-gray-700 mb-1">가입처</label>
              <input value={rentalCompany} onChange={(e)=>setRentalCompany(e.target.value)} placeholder="예: 세일즈파트너" title="가입처" className="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-transparent" />
            </div>
            <div>
              <label className="block text-xs font-medium text-gray-700 mb-1">상품명</label>
              <input value={productName} onChange={(e)=>setProductName(e.target.value)} placeholder="예: 즉시가입 패키지" title="상품명" className="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-transparent" />
            </div>
            <div>
              ...
              >
                ...
                <option key="immediate-join-일시납" value="일시납">일시납</option>
                <option key="immediate-join-1년납" value="1년납">1년납</option>
                <option key="immediate-join-2년납" value="2년납">2년납</option>
                <option key="immediate-join-3년납" value="3년납">3년납</option>
                <option key="immediate-join-5년납" value="5년납">5년납</option>
                <option key="immediate-join-7년납" value="7년납">7년납</option>
                <option key="immediate-join-10년납" value="10년납">10년납</option>
                <option key="immediate-join-15년납" value="15년납">15년납</option>
                <option key="immediate-join-20년납" value="20년납">20년납</option>
                <option key="immediate-join-25년납" value="25년납">25년납</option>
                <option key="immediate-join-30년납" value="30년납">30년납</option>
                <option key="immediate-join-전기납" value="전기납">전기납</option>
              </select>
            </div>
            <div>
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-gray-50">
              <tr>
                <th className="px-3 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">렌탈사</th>
              ...
              {rows.length === 0 ? (
                ...
                rows.map(r => (
                  <tr key={r.id} className="hover:bg-gray-50">
                    <td className="px-3 py-2 text-sm text-gray-900 whitespace-nowrap">{r.rentalCompany}</td>
...
export default ImmediateJoinItemsPage;
