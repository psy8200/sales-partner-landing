...
const ShoppingMallItemsPage = () => {
  ...
  const [pageTitle, setPageTitle] = useState('쇼핑몰구매설정'); // 기본값
  ...
  const [rows, setRows] = useState<ShoppingMallRow[]>([]);

  // 페이지 제목을 서버에서 가져오기 (하드코딩된 제목)
  React.useEffect(() => {
    fetch('/api/admin/sidebar-items/%2Fadmin%2Fitems%2Fshopping-mall')
      .then(r => r.json())
      .then(d => {
  ...
  React.useEffect(() => {
    fetch('/api/admin/items/settings?category=SHOPPING_MALL')
      .then(r=>r.json())
      .then(d=>{
        if (d?.items) {
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white shadow rounded-lg">
        <div className="p-4 space-y-4">
          {/* 1줄: 쇼핑몰 / 상품명 / 납입기간 / 기준금액 / 지급율 */}
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3">
            <div>
              <label className="block text-xs font-medium text-gray-700 mb-1">쇼핑몰</label>
              <input value={rentalCompany} onChange={(e)=>setRentalCompany(e.target.value)} placeholder="예: 세일즈쇼핑몰" title="쇼핑몰" className="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-transparent" />
            </div>
            <div>
              <label className="block text-xs font-medium text-gray-700 mb-1">상품명</label>
              <input value={productName} onChange={(e)=>setProductName(e.target.value)} placeholder="예: 쇼핑몰구매설정 패키지" title="상품명" className="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-transparent" />
            </div>
            <div>
              ...
              >
                ...
                <option key="shopping-mall-일시납" value="일시납">일시납</option>
                <option key="shopping-mall-1년납" value="1년납">1년납</option>
                <option key="shopping-mall-2년납" value="2년납">2년납</option>
                <option key="shopping-mall-3년납" value="3년납">3년납</option>
                <option key="shopping-mall-5년납" value="5년납">5년납</option>
                <option key="shopping-mall-7년납" value="7년납">7년납</option>
                <option key="shopping-mall-10년납" value="10년납">10년납</option>
                <option key="shopping-mall-15년납" value="15년납">15년납</option>
                <option key="shopping-mall-20년납" value="20년납">20년납</option>
                <option key="shopping-mall-25년납" value="25년납">25년납</option>
                <option key="shopping-mall-30년납" value="30년납">30년납</option>
                <option key="shopping-mall-전기납" value="전기납">전기납</option>
              </select>
            </div>
            <div>
...
export default ShoppingMallItemsPage;