...
const FuneralItemsPage = () => {
  ...
  const [pageTitle, setPageTitle] = useState('상조+결합상품설정'); // 기본값
  ...
  const [rows, setRows] = useState<FuneralRow[]>([]);

  // 페이지 제목을 서버에서 가져오기 (하드코딩된 제목)
  React.useEffect(() => {
    fetch('/api/admin/sidebar-items/%2Fadmin%2Fitems%2Ffuneral')
      .then(r => r.json())
      .then(d => {
  ...
  React.useEffect(() => {
    fetch('/api/admin/items/settings?category=FUNERAL')
      .then(r=>r.json())
      .then(d=>{
        if (d?.items) {
          setRows(d.items.map((x: ItemSetting)=>({
            id: x.id,
            funeralCompany: x.provider,
  ...
  const handleAdd = () => {
    if (!funeralCompany.trim() || !productName.trim() || !paymentTerm.trim()) {
      alert('상조사, 상품명, 납입기간을 입력해주세요.');
      return;
    }
    ...
    fetch('/api/admin/items/settings', {
      ...
      if (d?.item) {
        setRows(prev=>[{
          id: d.item.id,
          funeralCompany: d.item.provider,
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white shadow rounded-lg">
        <div className="p-4 space-y-4">
          {/* 1줄: 상조사 / 상품명 / 납입기간 / 기준금액 / 지급율 */}
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3">
            <div>
              <label className="block text-xs font-medium text-gray-700 mb-1">상조사</label>
              <input value={funeralCompany} onChange={(e)=>setFuneralCompany(e.target.value)} placeholder="예: 세일즈상조" title="상조사" className="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-transparent" />
            </div>
            <div>
              <label className="block text-xs font-medium text-gray-700 mb-1">상품명</label>
              <input value={productName} onChange={(e)=>setProductName(e.target.value)} placeholder="예: 상조보험 패키지" title="상품명" className="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-transparent" />
            </div>
            <div>
              ...
              >
                ...
                <option key="funeral-일시납" value="일시납">일시납</option>
                <option key="funeral-1년납" value="1년납">1년납</option>
                <option key="funeral-2년납" value="2년납">2년납</option>
                <option key="funeral-3년납" value="3년납">3년납</option>
                <option key="funeral-5년납" value="5년납">5년납</option>
                <option key="funeral-7년납" value="7년납">7년납</option>
                <option key="funeral-10년납" value="10년납">10년납</option>
                <option key="funeral-15년납" value="15년납">15년납</option>
                <option key="funeral-20년납" value="20년납">20년납</option>
                <option key="funeral-25년납" value="25년납">25년납</option>
                <option key="funeral-30년납" value="30년납">30년납</option>
                <option key="funeral-전기납" value="전기납">전기납</option>
              </select>
            </div>
            <div>
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-gray-50">
              <tr>
                <th className="px-3 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">상조사</th>
              ...
              {rows.length === 0 ? (
                ...
                rows.map(r => (
                  <tr key={r.id} className="hover:bg-gray-50">
                    <td className="px-3 py-2 text-sm text-gray-900 whitespace-nowrap">{r.funeralCompany}</td>
...
export default FuneralItemsPage;
